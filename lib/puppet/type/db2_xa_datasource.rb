...
Puppet::Type.newtype(:db2_xa_datasource) do
  ...
  newproperty(:server_port) do
    ...
    validate do |value|
      unless value == :nil or String(value) =~ /^[1-9][0-9]*$/
        raise ArgumentError , "#{value} is not a valid 'server_port' value (not an integer)."
      end
    end
    # If not undefined, convert to Integer
    munge do |value|
  ...
  newproperty(:valid_connection_checker_class_name) do
    ...
    newvalues("org.jboss.jca.adapters.jdbc.extensions.db2.DB2ValidConnectionChecker",
              :nil)
    defaultto {"org.jboss.jca.adapters.jdbc.extensions.db2.DB2ValidConnectionChecker"}
    # Convert Raw data to Typed data
    munge do |value|
      return value if value == :nil
      return String(value)
