...
Puppet::Type.newtype(:oracle_xa_datasource) do
  @doc = "Manages Oracle xa datasources via JBoss-cli.sh"
  ...
  newproperty(:url) do
    ...
    validate do |value|
      unless value =~ /^jdbc:oracle:\w*:@[0-9a-zA-Z]+([\-.][0-9a-zA-Z]+)*:\d{3,}:[0-9a-zA-Z]+$/
  ...
  newparam(:valid_connection_checker_class_name) do
    ...
    defaultto "org.jboss.jca.adapters.jdbc.extensions.oracle.OracleValidConnectionChecker"
    newvalues("org.jboss.jca.adapters.jdbc.extensions.oracle.OracleValidConnectionChecker")
  end
