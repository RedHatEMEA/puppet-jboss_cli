...
Puppet::Type.type(:mssql_xa_datasource).provide(:mssql_xa_ds) do
  ...
  def build_attrs_to_add()
    ...
    fail("Attribute 'server_name' is necessary for the 'create' operation to succeed.") if @resource[:server_name] == :nil
    fail("Attribute 'database_name' is necessary for the 'create' operation to succeed.") if @resource[:database_name] == :nil
    ...
    to_add["xa-datasource-properties"]["DriverType"] = {}
    to_add["xa-datasource-properties"]["DriverType"]["value"] = "4"
    to_add["xa-datasource-properties"]["ServerName"] = {}
    to_add["xa-datasource-properties"]["ServerName"]["value"] = @resource[:server_name]
    to_add["xa-datasource-properties"]["PortNumber"] = {}
    to_add["xa-datasource-properties"]["PortNumber"]["value"] = @resource[:server_port]
    to_add["xa-datasource-properties"]["DatabaseName"] = {}
    to_add["xa-datasource-properties"]["DatabaseName"]["value"] = @resource[:database_name]
  ...
  def server_name
    ...
    return :nil if $current_attrs["xa-datasource-properties"]["ServerName"] == :nil
    return $current_attrs["xa-datasource-properties"]["ServerName"]["value"]
  end

  def server_name=(new_value)
    $attrs_to_add["xa-datasource-properties"] = {} if $attrs_to_add["xa-datasource-properties"] == nil
    $attrs_to_add["xa-datasource-properties"]["ServerName"] = {} if $attrs_to_add["xa-datasource-properties"]["ServerName"] == nil
    $attrs_to_add["xa-datasource-properties"]["ServerName"]["value"] = new_value
  end

  def server_port
    return :nil if $current_attrs["xa-datasource-properties"] == :nil
    return :nil if $current_attrs["xa-datasource-properties"]["PortNumber"] == :nil
    return $current_attrs["xa-datasource-properties"]["PortNumber"]["value"]
  end

  def server_port=(new_value)
    $attrs_to_add["xa-datasource-properties"] = {} if $attrs_to_add["xa-datasource-properties"] == nil
    $attrs_to_add["xa-datasource-properties"]["PortNumber"] = {} if $attrs_to_add["xa-datasource-properties"]["PortNumber"] == nil
    $attrs_to_add["xa-datasource-properties"]["PortNumber"]["value"] = new_value
  end

  def database_name
    return :nil if $current_attrs["xa-datasource-properties"] == :nil
    return :nil if $current_attrs["xa-datasource-properties"]["DatabaseName"] == :nil
    return $current_attrs["xa-datasource-properties"]["DatabaseName"]["value"]
  end

  def database_name=(new_value)
    $attrs_to_add["xa-datasource-properties"] = {} if $attrs_to_add["xa-datasource-properties"] == nil
    $attrs_to_add["xa-datasource-properties"]["DatabaseName"] = {} if $attrs_to_add["xa-datasource-properties"]["DatabaseName"] == nil
    $attrs_to_add["xa-datasource-properties"]["DatabaseName"]["value"] = new_value
  end
